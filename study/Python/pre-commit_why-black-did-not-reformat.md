...
2. `pre-commit` isn't on PATH at all, and neither is `black` — not globally, not
   in `studyIterations/venvs/mps-py312/bin/`.
...
> **Why not `pip install` into the venv?** `studyIterations/venvs/mps-py312/`
...
`uv pip install --python studyIterations/venvs/mps-py312 'black==26.1.0'`.)
