...
#    Alternative that needs no hand-maintained list: see "Avoiding the
#    hand-maintained list" below — freeze the working venv instead.)
...
## Where that package list came from

Not from first principles — from the root `.venv` that already worked:

1. `uv pip freeze` on it → the 91 packages it had accumulated. That env was itself
   built empirically: run an import, read the `ModuleNotFoundError`, install what
   it names, repeat.
2. Filter those 91 down to the ~29 **direct** ones, cross-referencing the import
   block of `python/sglang/srt/entrypoints/http_server.py` against the record of
   which packages were added deliberately. The rest are transitive:
   `huggingface-hub` / `tokenizers` / `safetensors` / `regex` ride along with
   transformers, `httpx` / `jiter` / `distro` with openai, `typer` / `rich` /
   `markdown-it-py` with the HF CLI.
3. Sanity check: installing just the 29 names resolved to **exactly the same 91
   packages at the same versions**. So the list is neither short nor bloated.

It is authoritative only about *this* env — a set shaped by which files got
opened. It is not "sglang's macOS dependency set".

## Avoiding the hand-maintained list

Four options, all tried on 2026-08-17.

### A. Freeze the working env — recommended

```bash
VIRTUAL_ENV="$REPO/.venv" uv pip freeze | grep -v '^-e ' > study/Python/requirements-macos-dev.txt
# other machine:
VIRTUAL_ENV="$REPO/python/.venv" uv pip install -r study/Python/requirements-macos-dev.txt
```

Two confirmed caveats:

- The `grep -v '^-e '` is **required** — freeze emits
  `-e file:///Users/WChen/.../rust/sglang-server`, a machine-absolute path.
- **A pinned freeze is locked to its Python version.** Replaying the 3.14 freeze
  on 3.11 fails outright:
  `numpy==2.5.2 depends on Python>=3.12`. Same-version replay (3.14 → 3.14)
  works. To cross versions, strip the pins with `sed 's/==.*//'` (verified to
  resolve on 3.11) and then re-add `transformers==5.12.1` by hand.

Keep the same Python version on both machines and you keep the pins.

### B. Let the ImportError drive it

How the env was built in the first place, and genuinely list-free: run
`python -c "import sglang.srt.entrypoints.http_server"`, install whatever module
it names, repeat until quiet. Only friction is module name ≠ package name for a
few — `zmq` → `pyzmq`, `PIL` → `pillow`, `cv2` → `opencv-python`.

### C. `uv pip install -e python --no-deps` — fails on this machine

Appealing in theory: installs sglang itself (so no `.pth` needed) while skipping
the CUDA deps. In practice the build compiles the rust extension in
`rust/sglang-server` via setuptools-rust, and that needs a newer toolchain than
this machine has (rustc 1.90.0 installed):

```
error: rustc 1.90.0 is not supported by the following packages:
  rustpython-ruff_python_ast@0.15.8 requires rustc 1.92
```

A `rustup update` would probably clear it, but `--no-deps` installs no runtime
deps, so you would still need A or B afterwards.

### D. Parse `[project].dependencies` minus the CUDA entries

Automatable, but not worth it — the exclusion list is a judgment call, and
pyproject pulls in plenty that is irrelevant to just editing code.
